...
const P = {
  ...
  Low:    { color: "#22C55E", light: "#DCFCE7", text: "#166634" },
};
...
const INIT = [
  { id:1, name:"Design wireframes",   who:"Aishah", priority:"High",   status:"Done",        pct:100, due:"2024-01-10", desc:"Create mockups for all main screens including dashboard and mobile views." },
  { id:2, name:"Database schema",     who:"Aishah", priority:"High",   status:"Done",        pct:100, due:"2024-01-15", desc:"" },
  { id:3, name:"Build REST API",      who:"Aishah", priority:"High",   status:"In Progress", pct:65,  due:"2024-02-01", desc:"Implement CRUD endpoints with auth middleware." },
  { id:4, name:"Write unit tests",    who:"Aishah", priority:"Medium", status:"In Progress", pct:40,  due:"2024-02-05", desc:"" },
  { id:5, name:"Frontend components", who:"Aishah", priority:"Medium", status:"To Do",       pct:0,   due:"2024-02-10", desc:"" },
  { id:6, name:"User authentication", who:"Aishah", priority:"High",   status:"Blocked",     pct:20,  due:"2024-01-30", desc:"Blocked waiting on OAuth credentials from client." },
  { id:7, name:"Payment integration", who:"Aishah", priority:"High",   status:"To Do",       pct:0,   due:"2024-02-15", desc:"" },
  { id:8, name:"Email notifications", who:"Aishah", priority:"Low",    status:"To Do",       pct:0,   due:"2024-02-20", desc:"" },
];
...
function AutoTextarea({ value, onChange, placeholder }) {
  ...
  return (
    <textarea ref={ref} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={1}
      style={{ width:"100%", resize:"none", overflow:"hidden", border:"none", outline:"none", background:"transparent", fontFamily:"inherit", fontSize:13, lineHeight:1.6, color: value ? "#374151" : "#94A3B8", padding:0, minHeight:22 }} />
  );
}
      ...
      {options.map((opt, i) => {
        ...
        return (
          <div key={opt} style={{ height:24, borderRadius:99, background:active?c.color:"transparent", boxShadow:active?`0 1px 6px ${c.color}50`:"none", transition:"all 0.32s cubic-bezier(0.34,1.4,0.64,1)", display:"flex", alignItems:"center", justifyContent:"center", gap:active?5:0, flex:active?"1 1 auto":"0 0 18px", overflow:"hidden", padding:active?"0 10px":0 }}>
            ...
            <span style={{ fontSize:10, fontWeight:700, color:"#fff", whiteSpace:"nowrap", opacity:active?1:0, maxWidth:active?120:0, overflow:"hidden", transition:"opacity 0.22s, max-width 0.32s cubic-bezier(0.34,1.4,0.64,1)" }}>{opt}</span>
          </div>
        );
      })}
    </div>
...
function IOSSwitch({ on, onChange }) {
  return (
    <div onClick={onChange} style={{ width:40, height:24, borderRadius:99, background:on?"#22C55E":"#CBD5E1", position:"relative", cursor:"pointer", flexShrink:0, transition:"background 0.22s", boxShadow:on?"0 0 0 3px #22C55E28":"none" }}>
      <div style={{ position:"absolute", top:2, left:on?18:2, width:20, height:20, borderRadius:"50%", background:"#fff", boxShadow:"0 1px 4px rgba(0,0,0,0.2)", transition:"left 0.22s cubic-bezier(0.34,1.5,0.64,1)" }} />
    </div>
  );
}
...
function DateCell({ value, onChange }) {
  ...
  useEffect(() => {
    ...
    el.focus(); try { el.showPicker(); } catch { el.click(); }
  }, [editing]);
  return (
    <div style={{ position:"relative", height:26, display:"flex", alignItems:"center" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, background:"#fff", borderRadius:10, boxShadow:"0 8px 28px rgba(0,0,0,0.14)", border:"1px solid #E2E8F0", zIndex:50, minWidth:160, overflow:"hidden" }}>
          {assignees.map(a => (
            <div key={a} onClick={()=>{ onChange(a); setOpen(false); }}
              style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 12px", cursor:"pointer", background:a===value?"#F0F4FF":"transparent" }}
...
function TaskCard({ task, assignees, onUpd, onDel, onManage }) {
  ...
  return (
    <div style={{ background:isDone?"#F0FDF4":"#fff", borderRadius:16, boxShadow:"0 2px 12px rgba(0,0,0,0.07)", border:"1px solid #E2E8F0", overflow:"hidden" }}>
      {/* Card header — tap to expand */}
      <div onClick={()=>setExpanded(e=>!e)} style={{ padding:"14px 16px", display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}>
        <div style={{ flex:1, minWidth:0 }}>
          <EditableText value={task.name} onChange={v=>onUpd({name:v})}
            style={{ fontSize:14, fontWeight:700, color:isDone?"#94A3B8":"#1E293B", textDecoration:isDone?"line-through":"none" }} />
          <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:6, flexWrap:"wrap" }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"2px 8px", borderRadius:99, background:S[task.status].light }}>
              <div style={{ width:5, height:5, borderRadius:"50%", background:S[task.status].color }} />
              <span style={{ fontSize:10, fontWeight:700, color:S[task.status].text }}>{task.status}</span>
            </div>
            <div style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"2px 8px", borderRadius:99, background:P[task.priority].light }}>
              <span style={{ fontSize:10, fontWeight:700, color:P[task.priority].text }}>{task.priority}</span>
            </div>
            {task.due && <span style={{ fontSize:10, color:"#94A3B8" }}>📅 {fmt(task.due)}</span>}
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
          <IOSSwitch on={isDone} onChange={e=>{e.stopPropagation();onUpd({status:isDone?"To Do":"Done",pct:isDone?0:100});}} />
          <span style={{ fontSize:12, color:expanded?"#6366F1":"#CBD5E1", transition:"color 0.15s" }}>{expanded?"▲":"▼"}</span>
        </div>
      </div>
      ...
      {/* Expanded section */}
      {expanded && (
        <div style={{ borderTop:"1px solid #F1F5F9", padding:"14px 16px", display:"flex", flexDirection:"column", gap:14 }}>
          {/* Status + Priority toggles */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div>
              <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:5 }}>Status</p>
              <SegToggle options={STATUSES} value={task.status} config={S} onChange={s=>onUpd({status:s, pct:s==="Done"?100:s==="To Do"?0:task.pct})} />
            </div>
            <div>
              <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:5 }}>Priority</p>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, alignItems:"center" }}>
            <div>
              <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:5 }}>Assignee</p>
              <AssigneeCell value={task.who} assignees={assignees} onChange={w=>onUpd({who:w})} onManage={onManage} />
            </div>
            <div>
              <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:5 }}>Due date</p>
          ...
          {/* Description */}
          <div>
            <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:5 }}>Description</p>
            <div style={{ borderLeft:"2px solid #E2E8F0", paddingLeft:10 }}>
              <AutoTextarea value={task.desc||""} onChange={v=>onUpd({desc:v})} placeholder="Add a description… no limit!" />
            </div>
          </div>

          {/* Delete */}
          <button onClick={onDel} style={{ alignSelf:"flex-start", padding:"6px 14px", borderRadius:99, border:"none", background:"#FEE2E2", color:"#EF4444", fontSize:12, fontWeight:700, cursor:"pointer" }}>
            🗑 Delete task
          </button>
        </div>
      )}
    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:"28px", width:"100%", maxWidth:380, boxShadow:"0 24px 64px rgba(0,0,0,0.28)" }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:20 }}>
          <input ref={ref} value={name} onChange={e=>setName(e.target.value)} placeholder="Add a name…" onKeyDown={e=>e.key==="Enter"&&add()}
...
function AddModal({ onAdd, onClose, assignees }) {
  const [form, setForm] = useState({ name:"", desc:"", who:"Aishah", priority:"Medium", status:"To Do", pct:0, due:"" });
  const f = k => v => setForm(x=>({...x,[k]:v}));
  const valid = form.name.trim().length > 0;
  const submit = () => { if(valid){ onAdd({...form, id:uid()}); onClose(); } };
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(4px)", padding:20 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:"28px", width:"100%", maxWidth:440, boxShadow:"0 24px 64px rgba(0,0,0,0.28)", maxHeight:"90vh", overflowY:"auto" }}>
        ...
        <div style={{ border:"1.5px solid #E2E8F0", borderRadius:9, padding:"9px 12px", background:"#F8FAFC", marginBottom:12 }}>
          <AutoTextarea value={form.desc} onChange={f("desc")} placeholder="Add more details… (no limit)" />
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
          <div><label style={lbl}>Assignee</label>
            <select value={form.who} onChange={e=>f("who")(e.target.value)} style={inp}>{assignees.map(a=><option key={a}>{a}</option>)}</select></div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:22 }}>
          <div><label style={lbl}>Priority</label>
            <select value={form.priority} onChange={e=>f("priority")(e.target.value)} style={inp}>{PRIORITIES.map(p=><option key={p}>{p}</option>)}</select></div>
          <div><label style={lbl}>Status</label>
            <select value={form.status} onChange={e=>f("status")(e.target.value)} style={inp}>{STATUSES.map(s=><option key={s}>{s}</option>)}</select></div>
        </div>
        <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
...
export default function App() {
  ...
  const setStatus    = (id, s)    => upd(id, { status:s, pct:s==="Done"?100:s==="To Do"?0:tasks.find(t=>t.id===id).pct });
  const setPrio      = (id, p)    => upd(id, { priority:p });
  const setPct       = (id, v)    => upd(id, { pct:v, status:v===100?"Done":v===0?"To Do":"In Progress" });
  const toggleDone   = id         => { const t=tasks.find(x=>x.id===id),on=t.status!=="Done"; upd(id,{status:on?"Done":"To Do",pct:on?100:0}); };
  const addTask      = async form => {
    const { data, error } = await supabase.from("tasks").insert([{ name:form.name, desc:form.desc||"", who:form.who, priority:form.priority, status:form.status, pct:form.pct, due:form.due||null }]).select();
    if (!error && data) setTasks(t=>[...t, data[0]]);
  };
  const delTask      = async id   => { setTasks(t=>t.filter(x=>x.id!==id)); if(expandedId===id) setExpandedId(null); await supabase.from("tasks").delete().eq("id", id); };
  const saveAssignees= newList    => { setAssignees(newList); tasks.forEach(t=>{ if(!newList.includes(t.who)) upd(t.id,{who:newList[0]}); }); };
  const toggleExpand = (e, id)   => { e.stopPropagation(); setExpandedId(prev=>prev===id?null:id); };
  ...
  const DCOL    = "28px 1fr 110px 44px minmax(165px,1.2fr) minmax(120px,0.8fr) 120px 130px 32px 28px";
    ...
    <div style={{ minHeight:"100vh", background:"#0F172A", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", paddingBottom:48 }}>
      <style>{`
        *{box-sizing:border-box;margin:0}
        .drow{transition:background 0.15s;cursor:pointer}
        .drow:hover{background:#F5F3FF !important}
        .drow:hover .del-btn{opacity:1 !important}
      `}</style>

      {/* ── Header ── */}
      <div style={{ background:"linear-gradient(135deg,#4F46E5,#7C3AED)", padding: isMobile?"18px 16px 16px":"26px 28px 22px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
            <div>
              ...
              {!isMobile && <p style={{ fontSize:11, color:"#C7D2FE", marginTop:4 }}>Click row ▼ to expand description · Slide toggles · Drag progress</p>}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ background:"rgba(255,255,255,0.12)", borderRadius:12, padding:isMobile?"10px 14px":"13px 20px", textAlign:"center", border:"1px solid rgba(255,255,255,0.18)" }}>
                <p style={{ fontSize:isMobile?22:28, fontWeight:800, color:"#fff", lineHeight:1 }}>{donePct}%</p>
                <p style={{ fontSize:9, color:"#A5B4FC", fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", margin:"3px 0 7px" }}>Done</p>
          ...
          <div style={{ display:"flex", gap:isMobile?6:8, marginTop:14, flexWrap:"wrap" }}>
            {[
              ...
              {l:isMobile?"Prog":("In Progress"),v:counts["In Progress"],c:"#D97706",bg:"#FEF9C3",f:"In Progress"},
              ...
              return (
                <div key={x.l} onClick={()=>setFilter(x.f)}
                  style={{ background:x.bg, borderRadius:10, padding:isMobile?"7px 10px":"8px 14px", flex:1, minWidth:isMobile?48:65, cursor:"pointer", border:`2px solid ${active?x.c:x.c+"22"}`, boxShadow:active?`0 4px 14px ${x.c}33`:"none", transform:active?"translateY(-2px)":"none", transition:"all 0.18s" }}>
      ...
      {/* ── Filter bar ── */}
      <div style={{ background:"#1E293B", padding:isMobile?"8px 12px":"9px 28px", overflowX:"auto" }}>
        <div style={{ maxWidth:1200, margin:"0 auto", display:"flex", gap:6, alignItems:"center", minWidth:"max-content" }}>
          ...
          {["All",...STATUSES].map(s=>{
            ...
            return (
              <button key={s} onClick={()=>setFilter(s)} style={{ padding:"4px 12px", borderRadius:99, border:"none", cursor:"pointer", fontSize:11, fontWeight:700, transition:"all 0.18s", background:active?(cfg?.light||"#EEF2FF"):"rgba(255,255,255,0.07)", color:active?(cfg?.text||"#4F46E5"):"#64748B", outline:active?`1.5px solid ${cfg?.color||"#6366F1"}44`:"none", fontFamily:"inherit", whiteSpace:"nowrap" }}>
      ...
      <div style={{ maxWidth:1200, margin:"20px auto 0", padding:isMobile?"0 12px":"0 20px" }}>
        ...
        {loading ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            ...
            {shown.map(task => (
              <TaskCard key={task.id} task={task} assignees={assignees}
                onUpd={patch=>upd(task.id,patch)}
                onDel={()=>delTask(task.id)}
                onManage={()=>setShowMgr(true)}
              />
            ))}
            <div onClick={()=>setShowAdd(true)}
              style={{ padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"center", gap:8, cursor:"pointer", color:"#6366F1", fontSize:13, fontWeight:700, borderRadius:14, border:"2px dashed #C7D2FE", background:"#fff" }}>
              ➕ Add a task
            </div>
          </div>
        ) : (
          ...
          <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 4px 28px rgba(0,0,0,0.14)" }}>
            <div style={{ display:"grid", gridTemplateColumns:DCOL, padding:"9px 16px", background:"#F8FAFC", borderBottom:"1.5px solid #E2E8F0", gap:10, alignItems:"center" }}>
            ...
            {shown.map((task, i) => {
              ...
              const rowBg    = isDone?"#F0FDF4":i%2===0?"#fff":"#FAFBFE";
              const hasDesc  = task.desc && task.desc.trim().length > 0;
              return (
                <div key={task.id} style={{ borderBottom:"1px solid #F1F5F9" }}>
                  <div className="drow" onClick={e=>toggleExpand(e,task.id)}
                    style={{ display:"grid", gridTemplateColumns:DCOL, padding:"11px 16px", gap:10, background:rowBg, alignItems:"center" }}>
                    <span style={{ fontSize:10, color:"#CBD5E1", fontWeight:700, fontFamily:"monospace" }}>{String(i+1).padStart(2,"00")}</span>
                    <EditableText value={task.name} onChange={v=>upd(task.id,{name:v})}
                      style={{ fontSize:13, fontWeight:600, color:isDone?"#94A3B8":"#1E293B", textDecoration:isDone?"line-through":"none", transition:"all 0.3s" }} />
                    <AssigneeCell value={task.who} assignees={assignees} onChange={w=>upd(task.id,{who:w})} onManage={()=>setShowMgr(true)} />
                    <div onClick={e=>{e.stopPropagation();toggleDone(task.id);}}><IOSSwitch on={isDone} onChange={()=>{}} /></div>
                    <div onClick={e=>e.stopPropagation()}><SegToggle options={STATUSES} value={task.status} config={S} onChange={s=>setStatus(task.id,s)} /></div>
                    <div onClick={e=>e.stopPropagation()}><SegToggle options={PRIORITIES} value={task.priority} config={P} onChange={p=>setPrio(task.id,p)} /></div>
                    <div onClick={e=>e.stopPropagation()}><ProgBar value={task.pct} onChange={v=>setPct(task.id,v)} /></div>
                    <DateCell value={task.due} onChange={v=>upd(task.id,{due:v})} />
                    <div onClick={e=>toggleExpand(e,task.id)} title={expanded?"Collapse":"Expand description"}
                      style={{ display:"flex", alignItems:"center", justifyContent:"center", width:24, height:24, borderRadius:6, background:expanded?"#EEF2FF":hasDesc?"#F8FAFC":"transparent", cursor:"pointer", transition:"all 0.15s", flexShrink:0 }}>
                      <span style={{ fontSize:11, color:expanded?"#6366F1":hasDesc?"#94A3B8":"#CBD5E1" }}>{expanded?"▲":"▼"}</span>
                    </div>
                    <button className="del-btn" onClick={e=>{e.stopPropagation();delTask(task.id);}} title="Delete"
                  ...
                  {/* Description expand */}
                  {expanded && (
                    <div style={{ padding:"0 16px 14px 54px", background:rowBg, borderTop:"1px dashed #F1F5F9" }}>
                      <div style={{ borderLeft:"2px solid #E2E8F0", paddingLeft:12 }}>
                        <p style={{ fontSize:9, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:6 }}>Description</p>
                        <AutoTextarea value={task.desc||""} onChange={v=>upd(task.id,{desc:v})} placeholder="Add a description… no limit, write as much as you want!" />
                      </div>
                    </div>
                  )}
                </div>
        ...
        <p style={{ textAlign:"center", fontSize:10, color:"#94A3B8", marginTop:10, opacity:0.4 }}>
          {isMobile ? "Tap any card to expand · Add description inside" : "Click any row ▼ to expand description · Hover to delete"}
        </p>
      </div>
